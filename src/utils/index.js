...
export const parseStringValue = value => (/^(true|false)$/.test(value) ? JSON.parse(value) : value);
...
export function transformYesAndNo(data) {
  const newData = JSON.parse(JSON.stringify(data));
  // eslint-disable-next-line guard-for-in
  for (const prop in newData) {
    if (newData[prop] === 'yes') newData[prop] = true;
    if (newData[prop] === 'no') newData[prop] = false;
  }
  return newData;
}
...
export function getTimeLeft(miliseconds) {
  if (!miliseconds) {
    return '-';
  }
  const startDate = new Date();
  const endDate = new Date(miliseconds * 1000);
  ...
  return `${diffHrs > 0 ? `${diffHrs} h ` : ''}${diffMins > 0 ? `${diffMins} min` : ''}`;
}

export function formatDateNews(miliseconds) {
  if (!miliseconds) {
    return '-';
  }
  // eslint-disable-next-line
...
export function clearString(str) {
  // eslint-disable-next-line
  return str.replace(/[^A-Za-z0-9\[\] ]/g, (a) => { return CH_MAP[a] || a; });
}

export function getFormConfigurationByMunicipality(municipality) {
  switch (municipality) {
    case MUNICIPALITY_CODE_CDMX:
      return FORM_CDMX;
    default:
      return FORM_BASE;
  }
